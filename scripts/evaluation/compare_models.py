...
def evaluate(path: Path, tag: str, split: str):
    ...
    per_class = {model.names[c]: (m.box.ap50[i], int(m.box.nt_per_class[c]))
...
def main():
    ...
    for c in CLASS_NAMES:
        support = next((r["per_class"][c][1] for r in results if c in r["per_class"]), 0)
        cells = "".join(f"{r['per_class'][c][0]:>12.3f}" if c in r["per_class"]
                        else f"{'--':>12s}" for r in results)
        flag = "  ← too few GT to trust" if 0 < support < 20 else ("  ← ABSENT" if support == 0 else "")
        print(f"{c:<16s}{cells}{support:>8d}{flag}")

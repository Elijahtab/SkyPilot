"""
⚠ ARCHIVED — DO NOT USE. Superseded by scripts/labeling/auto_label_kaggle.py.

This proposed boxes with stock yolov8n.pt (conf>=0.3) and then asked GPT-4o to
name each crop. The box proposals are far too incomplete: measured against
Vehicle_type_detection_v4, its 99-image output had class-agnostic precision
0.255 — v4 finds 3.6x more real vehicles than this script labeled, i.e. ~72% of
vehicles were left unlabeled and would train as background.

That output is quarantined at Labeling/quarantine_branchB_yolov8n/.
Lowering the confidence threshold does NOT fix it; the misses are capacity-
limited. Re-propose with a stronger detector (v4 itself scores 0.910 recall on
that imagery) if you revive this approach.
"""
